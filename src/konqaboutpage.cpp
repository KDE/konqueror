...
static QString loadFile(const QString &relativeFile)
{
    const QString file = QStandardPaths::locate(QStandardPaths::GenericDataLocation, relativeFile);
    if (file.isEmpty()) {
        qWarning() << "File not found:" << relativeFile;
    } else {
        QFile f(file);
        if (!f.open(QIODevice::ReadOnly)) {
            qWarning() << "Error opening" << file << ":" << f.errorString();
        } else {
...
static QString infoPageCssUrl()
{
    return QUrl::fromLocalFile(QStandardPaths::locate(QStandardPaths::GenericDataLocation, QStringLiteral("kf5/infopage/kde_infopage.css"))).toString();
}

static QString otherCssImports()
{
    QString imports;
    if (qApp->layoutDirection() == Qt::RightToLeft) {
        imports = QStringLiteral("@import \"%1\";").arg(QUrl::fromLocalFile(QStandardPaths::locate(QStandardPaths::GenericDataLocation, QStringLiteral("kf5/infopage/kde_infopage_rtl.css"))).toString());
    }
    imports += QStringLiteral("\n@import \"%1\";").arg(QUrl::fromLocalFile(QStandardPaths::locate(QStandardPaths::GenericDataLocation, QStringLiteral("konqueror/about/konq.css"))).toString());
    return imports;
}

QString KonqAboutPage::launch() const
{
    QString res = loadFile(QStringLiteral("konqueror/about/launch.html"));
    if (res.isEmpty()) {
        return res;
    }
    ...
    res = res.arg(infoPageCssUrl(), otherCssImports());
...
QString KonqAboutPage::intro() const
{
    QString res = loadFile(QStringLiteral("konqueror/about/intro.html"));
    if (res.isEmpty()) {
        return res;
    }
...
QString KonqAboutPage::specs() const
{
    KIconLoader *iconloader = KIconLoader::global();
    QString res = loadFile(QStringLiteral("konqueror/about/specs.html"));
    QString continue_icon_path = QUrl::fromLocalFile(iconloader->iconPath(QApplication::isRightToLeft() ? "go-previous" : "go-next", KIconLoader::Small)).toString();
    if (res.isEmpty()) {
        return res;
    }
...
QString KonqAboutPage::tips() const
{
    QString res = loadFile(QStringLiteral("konqueror/about/tips.html"));
    if (res.isEmpty()) {
        return res;
    }
...
QString KonqAboutPage::plugins() const
{
    QString res = loadFile(qApp->layoutDirection() == Qt::RightToLeft ? "konqueror/about/plugins_rtl.html" : "konqueror/about/plugins.html")
